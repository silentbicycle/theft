...
/* Compare A against B, for a known type. (Should cast internally.)
 * Should returns 0 if they match, or a non-zero value if they differ.
 * The non-zero value will be pased to the greatest_print_diff_cb
 * function, if present. */
typedef int (greatest_cmp_cb)(void *a, void *b);

/* Print a message indicating that A and B differ, in some
 * type-specific manner. CMP_RES is the output from greatest_cmp_cb,
 * which can be used to represent an offset.*/
typedef void (greatest_print_diff_cb)(void *a, void *b, int cmp_res);
...
#if 0
/* Various other EQ funs should spill into this. */
int greatest_assert_cb_eq(void *a, void *b,
    greatest_cmp_cb *cmp, greatest_print_diff_cb *print, void *print_udata,
        int invert, const char *file, unsigned int line) {
    int cmp_res = cmp(a, b);
    if (invert) {               /* expected to NOT match */
        if (res == 0) {
            greatest_info.fail_file = file;
            greatest_info.fail_line = line;
            greatest_info.msg = "Difference expected, matched";
            return -1;                                                      
        }
    } else {                    /* expected to match */
        if (cmp_res != 0) {
            /* print diff with print, if defined */
            if (print) {
                print(a, b, print_udata, cmp_res);
            }
            greatest_info.fail_file = file;
            greatest_info.fail_line = line;
            greatest_info.msg = "???";
...
void greatest_print_diff_num(void *va, void *vb, void *udata, int cmp_res) {
    int a = (int)va;
    int b = (int)vb;
    fprintf(GREATEST_STDOUT, "Expected %d, got %d\n", a, b);
}
#endif
